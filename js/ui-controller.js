...
function setTab(tab,opts){
  ...
  state.activeTab=tab;
  $$('#bottomNav .nav-btn').forEach(function(b){b.classList.toggle('is-active',b.getAttribute('data-tab')===tab)});
...
function wireNav(){
  ...
  $$('.quick-action').forEach(function(btn){btn.addEventListener('click',function(){setTab(btn.getAttribute('data-goto'))})});
}
...
function applyInitialRoute(){
  try{
    ...
    if(t&&['xhamia','namazi','rreth','galeria','cilesimet'].indexOf(t)>=0)setTab(t,{force:true});
